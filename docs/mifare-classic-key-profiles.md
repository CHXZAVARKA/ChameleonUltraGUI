...
A key profile keeps verified sector-key assignments without turning them into a
flat dictionary. The read-card flow checks the assigned profile first and uses
the selected dictionary only for unresolved Key A/Key B slots.
...
from the app's generic settings JSON/QR backup because that path is not a
credential export and can otherwise disclose keys together with sector and UID
assignments.

## Saved Cards integration

Profiles saved after a successful read are stored through the same local
preferences provider used by the read and write flows. They appear
automatically in the **Assigned key profiles** section of **Saved Cards**;
there is no separate move or registration step. That section is the shared
place to import, inspect, export, or delete profiles. The read-card and
standard-writer panels only select profiles already stored there; they do not
show a second import action.

The standard writer reads its profile dropdown from that same collection. Its
saved-dump button reuses the app's normal searchable saved-card selector, with
the source list filtered to complete MIFARE Classic dumps. It does not keep a
second list or a second saved-card picker.
