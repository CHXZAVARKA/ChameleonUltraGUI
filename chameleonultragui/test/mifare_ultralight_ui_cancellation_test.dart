...
void main() {
  ...
  testWidgets(
      'late Ultralight transport error cannot report through replacement model',
      (tester) async {
    final communicator = _DelayedUltralightCommunicator();
    final appState = _connectedState(preferences, communicator);
    final replacementInfo = HFCardInfo(type: TagType.ntag216);
    ...
    await _pumpHelper(tester, appState, replacementInfo);
    communicator.firstResponse.completeError(StateError('port closed'));
  ...
  testWidgets('late Ultralight transport error does not setState after dispose',
      (tester) async {
    final communicator = _DelayedUltralightCommunicator();
    final appState = _connectedState(preferences, communicator);
